...
public class GameTest {
    ...
    public void setUp() {
        ...
        // Create test items
        item1 = new Item("Flashlight", "A bright LED flashlight", "Illuminates dark areas");
    ...
    public void testConstructor_ValidSession() {
        ...
        assertEquals("Theme should match session theme", "Horror", testGame.getTheme());
        assertEquals("Difficulty should match session difficulty", 1, testGame.getDifficulty());
        assertEquals("Player count should match session player count", 2, testGame.getPlayerCount());
    ...
    public void testConstructor_DifferentDifficulties() {
        // Test Easy (1)
        GameSession easySession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 1);
        Game easyGame = new Game(easySession);
        assertEquals("Easy difficulty should be 1", 1, easyGame.getDifficulty());

        // Test Medium (2)
        GameSession mediumSession = new GameSession(testUserID, "Team", "Session", "Theme", 2, 1);
        Game mediumGame = new Game(mediumSession);
        assertEquals("Medium difficulty should be 2", 2, mediumGame.getDifficulty());

        // Test Hard (3)
        GameSession hardSession = new GameSession(testUserID, "Team", "Session", "Theme", 3, 1);
    ...
    public void testConstructor_VariousPlayerCounts() {
        for (int i = 1; i <= 10; i++) {
            GameSession testSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, i);
            Game testGame = new Game(testSession);
    ...
    public void testConstructor_NullTheme() {
        GameSession nullThemeSession = new GameSession(testUserID, "Team", "Session", null, 1, 1);
    ...
    public void testConstructor_EmptyTheme() {
        GameSession emptyThemeSession = new GameSession(testUserID, "Team", "Session", "", 1, 1);
    ...
    public void testGetTheme_DifferentThemes() {
        ...
        for (String theme : themes) {
            GameSession testSession = new GameSession(testUserID, "Team", "Session", theme, 1, 1);
            Game testGame = new Game(testSession);
    ...
    public void testGetDifficulty_BoundaryValues() {
        ...
        GameSession minSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 1);
        ...
        GameSession maxSession = new GameSession(testUserID, "Team", "Session", "Theme", 3, 1);
    ...
    public void testGetDifficulty_InvalidValues() {
        ...
        GameSession zeroSession = new GameSession(testUserID, "Team", "Session", "Theme", 0, 1);
        Game zeroGame = new Game(zeroSession);
        assertEquals("Zero difficulty should be stored", 0, zeroGame.getDifficulty());

        // Test negative difficulty
        GameSession negSession = new GameSession(testUserID, "Team", "Session", "Theme", -1, 1);
        Game negGame = new Game(negSession);
        assertEquals("Negative difficulty should be stored", -1, negGame.getDifficulty());
    }
    ...
    public void testGetPlayerCount_SinglePlayer() {
        GameSession singleSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 1);
        Game singleGame = new Game(singleSession);
        assertEquals("Single player count should be 1", 1, singleGame.getPlayerCount());
    }
    ...
    public void testGetPlayerCount_ManyPlayers() {
        GameSession manySession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 100);
        Game manyGame = new Game(manySession);
        assertEquals("Many players count should be 100", 100, manyGame.getPlayerCount());
    }
    ...
    public void testGetPlayerCount_ZeroPlayers() {
        GameSession zeroSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 0);
        Game zeroGame = new Game(zeroSession);
    ...
    public void testGetIndex_AfterChallengeStart() {
        ...
        assertEquals("Index should be 0 after challengeStart(0)", 0, game.getIndex());

        game.challengeStart(2);
        assertEquals("Index should be 2 after challengeStart(2)", 2, game.getIndex());
    }
    ...
    public void testGetIndex_NegativeStartingIndex() {
        ...
        assertEquals("Index should be -1 after challengeStart(-1)", -1, game.getIndex());
    }
    ...
    public void testGetChallenges_InitiallyEmpty() {
        assertTrue("Challenges list should be empty initially", game.getChallenges().isEmpty());
    }
    ...
    public void testSetGameSet_ValidTemplate() {
        ...
        game.assignChallenges();
        assertTrue("Challenges should be empty for new template", game.getChallenges().isEmpty());
    }
    ...
    public void testSetGameSet_ReplaceTemplate() {
        ...
        assertEquals("Should have 1 challenge after replacement", 1, game.getChallenges().size());
        assertEquals("Challenge should be horror challenge", horrorChallenge, game.getChallenges().get(0));
    }
    ...
    public void testChallengeStart_IndexBeyondSize() {
        ...
        assertEquals("Should still have 3 challenges", 3, game.getChallenges().size());
    }
    ...
    public void testAssignChallenges_MultipleTimes() {
        game.assignChallenges();
        assertEquals("Should have 3 challenges first time", 3, game.getChallenges().size());

        game.assignChallenges();
        assertEquals("Should still have 3 challenges second time", 3, game.getChallenges().size());
    }
    ...
    public void testGetScore_NonZeroInitialScore() {
        GameSession scoredSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 1);
    ...
    public void testGetScore_LargeScore() {
        GameSession largeSession = new GameSession(testUserID, "Team", "Session", "Theme", 1, 1);
    ...
    public void testIntegration_CompleteSetup() {
        ...
        GameTemplate template = new GameTemplate("Adventure", "Welcome to adventure!");
        ...
        assertEquals("Theme should be Adventure", "Adventure", integrationGame.getTheme());
        assertEquals("Difficulty should be 2", 2, integrationGame.getDifficulty());
        assertEquals("Player count should be 3", 3, integrationGame.getPlayerCount());
        assertEquals("Should have 2 challenges", 2, integrationGame.getChallenges().size());
    ...
    public void testIntegration_MultipleGamesIndependent() {
        GameSession session1 = new GameSession(testUserID, "Team1", "Session1", "Theme1", 1, 2);
        GameSession session2 = new GameSession(testUserID, "Team2", "Session2", "Theme2", 3, 4);
    ...
    public void testIntegration_MaximumConfiguration() {
        ...
        assertEquals("Should have 50 challenges", 50, maxGame.getChallenges().size());
    ...
    public void testIntegration_StateConsistency() {
        ...
        assertTrue("Initial challenges should be empty", game.getChallenges().isEmpty());
    ...
    public void testIntegration_ChallengesWithItemsAndHints() {
        ...
        assertEquals("Challenge 1 should have 2 hints", 2, challenges.get(0).getHints().size());
        assertEquals("Challenge 1 should have 1 item", 1, challenges.get(0).getItems().size());

        // Verify challenge 2 has hints but no items
        assertEquals("Challenge 2 should have 1 hint", 1, challenges.get(1).getHints().size());
        assertEquals("Challenge 2 should have 0 items", 0, challenges.get(1).getItems().size());

        // Verify challenge 3 has hints and items
        assertEquals("Challenge 3 should have 1 hint", 1, challenges.get(2).getHints().size());
        assertEquals("Challenge 3 should have 1 item", 1, challenges.get(2).getItems().size());
    }
    ...
    public void testIntegration_NullChallengeInTemplate() {
        ...
        assertEquals("Should have 2 elements (including null)", 2, game.getChallenges().size());
        assertNull("First element should be null", game.getChallenges().get(0));
        assertEquals("Second element should be challenge1", challenge1, game.getChallenges().get(1));
    }
    ...
    public void testIntegration_SessionWithHintsUsed() {
        GameSession hintSession = new GameSession(testUserID, "Team", "Session", "Theme", 2, 2);
        ...
        assertEquals("Session should have 5 hints used", 5, hintSession.getHintsUsed());
    }
    ...
    public void testIntegration_GameStateSummary() {
        ...
        String summary = String.format(
            "Theme: %s, Difficulty: %d, Players: %d, Index: %d, Score: %d, Challenges: %d",
            game.getTheme(),
            game.getDifficulty(),
            game.getPlayerCount(),
            game.getIndex(),
        ...
        assertEquals("Summary should match expected format",
            "Theme: Mystery, Difficulty: 2, Players: 4, Index: 1, Score: 0, Challenges: 3",
            summary);
    }
}
