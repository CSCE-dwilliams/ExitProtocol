...
// * Tests all methods including edge cases, error conditions, and data
// integrity.
// *
// * This test suite focuses on what the methods SHOULD return/do based on their
// * documentation and intended behavior, helping to identify bugs in the
// implementation.
// *
...
// * 5. getGameJSON() is incomplete - only sets difficulty field to game object
...
// private static final String BACKUP_USERS_FILE =
// "json/users_backup_writer.json";
...
// User testUser = new User("Test", "User", "test@test.com", "password", 1,
// UUID.randomUUID());
...
// assertEquals("Empty UserList should result in empty JSON array", 0,
// jsonArray.size());
// } catch (Exception e) {
...
// User user1 = new User("User1", "Last1", "user1@test.com", "pass1", 1,
// UUID.randomUUID());
// User user2 = new User("User2", "Last2", "user2@test.com", "pass2", 2,
// UUID.randomUUID());
// User user3 = new User("User3", "Last3", "user3@test.com", "pass3", 3,
// UUID.randomUUID());
...
// User testUser = new User("Round", "Trip", "roundtrip@test.com",
// "password123", 5, testUUID);
// userList.getUsers().add(testUser);
...
// assertEquals("Email should match", "roundtrip@test.com",
// loadedUser.getEmail());
// assertEquals("Password should match", "password123",
...
// // First save
// User user1 = new User("First", "User", "first@test.com", "pass", 1,
// UUID.randomUUID());
// userList.getUsers().add(user1);
// DataWriter.saveUsers();
...
// userList.getUsers().clear();
// User user2 = new User("Second", "User", "second@test.com", "pass", 2,
// UUID.randomUUID());
// userList.getUsers().add(user2);
// DataWriter.saveUsers();
...
// ArrayList<User> loadedUsers = userList.getUsers();
// assertEquals("Should have only 1 user after overwrite", 1,
// loadedUsers.size());
// assertEquals("Should be the second user", "Second",
// loadedUsers.get(0).getFirstName());
// }
...
// UUID testUUID = UUID.randomUUID();
// User testUser = new User("John", "Doe", "john@example.com", "secret123", 3,
// testUUID);
...
// assertTrue("Sessions should be a JSONArray", jsonObj.get("sessions")
...
// assertEquals("User with no sessions should have empty array", 0,
// sessions.size());
// }
...
// GameSession session = new GameSession(userUUID, "TestTeam", "TestSession",
// "Horror", 2, 3);
...
// * Session JSON should contain id, teamname, score, theme, difficulty,
// playercount,
...
// GameSession session = new GameSession(userUUID, "TeamAlpha", "Session1",
// "Mystery", 3, 4);
...
// assertTrue("Session should contain teamname",
// sessionObj.containsKey("teamname"));
...
// assertTrue("Session should contain difficulty",
// sessionObj.containsKey("difficulty"));
// assertTrue("Session should contain playercount",
// sessionObj.containsKey("playercount"));
// assertTrue("Session should contain currentChallengeIndex",
// sessionObj.containsKey("currentChallengeIndex"));
// assertTrue("Session should contain state", sessionObj.containsKey("state"));
// assertTrue("Session should contain sessionName",
// sessionObj.containsKey("sessionName"));
// assertTrue("Session should contain hintsUsed",
// sessionObj.containsKey("hintsUsed"));
// }
...
// GameSession session = new GameSession(userUUID, "TeamBeta", "Session2",
// "Fantasy", 2, 2);
...
// assertEquals("Session ID should match", sessionID.toString(),
// sessionObj.get("id"));
// assertEquals("Team name should match", "TeamBeta",
// sessionObj.get("teamname"));
...
// assertEquals("Challenge index should match", 6,
// sessionObj.get("currentChallengeIndex"));
// assertEquals("Session name should match", "Session2",
// sessionObj.get("sessionName"));
// assertEquals("Hints used should match", 4, sessionObj.get("hintsUsed"));
// assertEquals("State should be ACTIVE by default", "ACTIVE",
// sessionObj.get("state"));
// }
...
// GameSession session1 = new GameSession(userUUID, "Team1", "Session1",
// "Horror", 1, 2);
// GameSession session2 = new GameSession(userUUID, "Team2", "Session2",
// "Mystery", 2, 3);
// GameSession session3 = new GameSession(userUUID, "Team3", "Session3",
// "Fantasy", 3, 4);
...
// User testUser = new User("Test\"Quote", "User's", "test@test.com",
// "pass\"word", 1, UUID.randomUUID());
...
// GameSession session = new GameSession(userUUID, "Team", "Session", "Horror",
// 1, 2);
...
// UUID userUUID = UUID.randomUUID();
// User testUser = new User("Test", "User", "test@test.com", "password", 0,
// userUUID);
...
// assertEquals("Challenge index of 0 should be preserved", 0,
// sessionObj.get("currentChallengeIndex"));
// assertEquals("Hints used of 0 should be preserved", 0,
// sessionObj.get("hintsUsed"));
// }
...
// UUID userUUID = UUID.randomUUID();
// User testUser = new User("Test", "User", "test@test.com", "password", 999,
// userUUID);
...
// assertEquals("Large score should be preserved", 999999,
// sessionObj.get("score"));
// assertEquals("Large challenge index should be preserved", 1000,
// sessionObj.get("currentChallengeIndex"));
// assertEquals("Large hints used should be preserved", 500,
// sessionObj.get("hintsUsed"));
// }
...
// UUID userUUID = UUID.randomUUID();
// User originalUser = new User("Complete", "RoundTrip", "complete@test.com",
// "secure", 7, userUUID);

// GameSession session1 = new GameSession(userUUID, "TeamA", "SessionA",
// "Horror", 2, 3);
// session1.setScore(250);
// session1.setChallengeIndex(8);
// session1.setHintsUsed(5);

// GameSession session2 = new GameSession(userUUID, "TeamB", "SessionB",
// "Mystery", 3, 2);
...
// User loadedUser = loadedUsers.get(0);
// assertEquals("First name should match", "Complete",
// loadedUser.getFirstName());
// assertEquals("Last name should match", "RoundTrip",
// loadedUser.getLastName());
// assertEquals("Email should match", "complete@test.com",
// loadedUser.getEmail());
...
// assertEquals("Session1 team name should match", "TeamA",
// loadedSession1.getTeamName());
// assertEquals("Session1 score should match", 250, loadedSession1.getScore());
// assertEquals("Session1 challenge index should match", 8,
// loadedSession1.getChallengeIndex());
// assertEquals("Session1 hints used should match", 5,
// loadedSession1.getHintsUsed());

// assertEquals("Session2 team name should match", "TeamB",
// loadedSession2.getTeamName());
// assertEquals("Session2 score should match", 300, loadedSession2.getScore());
// assertEquals("Session2 challenge index should match", 10,
// loadedSession2.getChallengeIndex());
// assertEquals("Session2 hints used should match", 3,
// loadedSession2.getHintsUsed());
// }
...
// * NOTE: Current implementation appears incomplete - only sets difficulty
// field.
// */
// @Test
...
// * NOTE: This test documents what SHOULD be in the JSON, not what currently
// is.
...
// assertTrue("Should contain difficulty field",
// jsonObj.containsKey("difficulty"));

// // These SHOULD exist but currently don't:
// // assertTrue("Should contain theme", jsonObj.containsKey("theme"));
// // assertTrue("Should contain playerCount",
// jsonObj.containsKey("playerCount"));
...
// * NOTE: This test documents expected behavior, current implementation is
// incomplete.
// */
// @Test
...
// assertEquals("Empty first name should be preserved", "",
// jsonObj.get("firstname"));
// assertEquals("Empty last name should be preserved", "",
// jsonObj.get("lastname"));
// assertEquals("Empty email should be preserved", "", jsonObj.get("email"));
// assertEquals("Empty password should be preserved", "",
...
// User testUser = new User(longString, longString, longString, longString, 1,
// UUID.randomUUID());
// userList.getUsers().add(testUser);
...
// assertEquals("Multiple saves should produce identical results", firstSave,
// secondSave);
// }
...
// GameSession activeSession = new GameSession(userUUID, "Team1", "Active",
// "Horror", 1, 2);
// // activeSession is ACTIVE by default

// GameSession pausedSession = new GameSession(userUUID, "Team2", "Paused",
// "Mystery", 2, 3);
// pausedSession.pause();

// GameSession completedSession = new GameSession(userUUID, "Team3",
// "Completed", "Fantasy", 3, 4);
...
// // Verify that states are serialized (actual values depend on iteration
// order)
