...
// * Tests all public methods, edge cases, and integration with UserList,
...
// assertEquals("Should load exactly 4 test users", 4,
// leaderboard.getUsers().size());
// }
...
// assertNotNull("Leaderboard should be created even with empty data",
// emptyLeaderboard);
...
// leaderboard.reloadUsers();
// assertEquals("Should reload with updated data", 2,
// leaderboard.getUsers().size());
...
// User newUser = new User("John", "Doe", "john@test.com", "password", 1,
// UUID.randomUUID());
// leaderboard.addUser(newUser);

// assertEquals("User count should increase by 1", initialCount + 1,
// leaderboard.getUsers().size());
// assertTrue("New user should be in the list",
// leaderboard.getUsers().contains(newUser));
// }
...
// User user1 = new User("User", "One", "user1@test.com", "pass1", 1,
// UUID.randomUUID());
// User user2 = new User("User", "Two", "user2@test.com", "pass2", 2,
// UUID.randomUUID());

// leaderboard.addUser(user1);
// leaderboard.addUser(user2);

// assertEquals("Should add both users", initialCount + 2,
// leaderboard.getUsers().size());
// assertTrue("Should contain first user",
// leaderboard.getUsers().contains(user1));
// assertTrue("Should contain second user",
// leaderboard.getUsers().contains(user2));
// }
...
// assertEquals("Should still add user even with null fields", initialCount + 1,
// leaderboard.getUsers().size());
// assertTrue("User with null fields should be in list",
// leaderboard.getUsers().contains(userWithNulls));
// }
...
// assertEquals("First user should be Charlie with highest score", "Charlie",
// users.get(0).getFirstName());
// assertEquals("Second user should be Alice", "Alice",
// users.get(1).getFirstName());
// assertEquals("Third user should be Bob", "Bob", users.get(2).getFirstName());
// assertEquals("Fourth user should be Diana with no sessions", "Diana",
...
// User user1 = new User("Equal1", "Score", "equal1@test.com", "pass", 1,
// UUID.randomUUID());
// User user2 = new User("Equal2", "Score", "equal2@test.com", "pass", 2,
// UUID.randomUUID());

// // Create sessions with equal total scores
// GameSession session1 = new GameSession(user1.getUUID(), "Team1", "Session1",
// "Horror", 1, 1);
// session1.setScore(100);
// user1.storeGameSession(session1);

// GameSession session2 = new GameSession(user2.getUUID(), "Team2", "Session2",
// "Fantasy", 1, 1);
...
// leaderboard.sortByScore();
// assertEquals("Empty list should remain empty", 0,
// leaderboard.getUsers().size());
// }
...
// User singleUser = new User("Solo", "Player", "solo@test.com", "pass", 1,
// UUID.randomUUID());
...
// assertEquals("Should be the same user", "Solo",
// leaderboard.getUsers().get(0).getFirstName());
// }
...
// assertTrue("Should contain leaderboard header", output.contains("------
// Leaderboard ------"));
...
// assertTrue("Charlie should be ranked first", output.indexOf("Charlie Brown -
// 500") < output.indexOf("Alice Johnson"));

// // Alice should be second with 350 points (150 + 200)
// assertTrue("Alice should be ranked second", output.indexOf("Alice Johnson -
// 350") < output.indexOf("Bob Smith"));

// // Bob should be third with 100 points
// assertTrue("Bob should be ranked third", output.indexOf("Bob Smith - 100") <
// output.indexOf("Diana Prince"));

// // Diana should be last with 0 points (no sessions)
// assertTrue("Diana should be ranked last", output.contains("Diana Prince -
// 0"));
// }
...
// String output = outContent.toString();
// assertTrue("Should display header even when empty", output.contains("------
// Leaderboard ------"));
...
// assertSame("Should return reference to UserList users", userList.getUsers(),
// users);
// }
...
// User newUser = new User("Direct", "Add", "direct@test.com", "pass", 1,
// UUID.randomUUID());
// userList.getUsers().add(newUser);

// // getUsers should reflect this change
// assertEquals("getUsers should reflect UserList changes", initialSize + 1,
// leaderboard.getUsers().size());
// assertTrue("Should contain directly added user",
// leaderboard.getUsers().contains(newUser));
// }

// // ============ SCORE CALCULATION TESTS (PRIVATE METHOD TESTING VIA PUBLIC
// METHODS) ============
...
// User userA = new User("UserA", "Test", "a@test.com", "pass", 1,
// UUID.randomUUID());
// User userB = new User("UserB", "Test", "b@test.com", "pass", 2,
// UUID.randomUUID());

// // UserA: 2 sessions with scores 50 and 75 = 125 total
// GameSession sessionA1 = new GameSession(userA.getUUID(), "TeamA1",
// "SessionA1", "Horror", 1, 1);
// sessionA1.setScore(50);
// GameSession sessionA2 = new GameSession(userA.getUUID(), "TeamA2",
// "SessionA2", "Fantasy", 2, 1);
...
// GameSession sessionB1 = new GameSession(userB.getUUID(), "TeamB1",
// "SessionB1", "Mystery", 3, 1);
...
// assertEquals("UserB should be first with higher score", "UserB",
// leaderboard.getUsers().get(0).getFirstName());
// assertEquals("UserA should be second", "UserA",
// leaderboard.getUsers().get(1).getFirstName());
// }
...
// User userWithNoSessions = new User("NoSessions", "User",
// "nosessions@test.com", "pass", 1, UUID.randomUUID());
// userList.getUsers().add(userWithNoSessions);

// // Should not throw exception when calculating score
// leaderboard.sortByScore();
// assertEquals("User with no sessions should still be in list", 1,
// leaderboard.getUsers().size());

// // Test that the user is properly handled (the actual display method works,
// // but capturing output in tests is tricky - the core functionality is
// tested)
// assertEquals("User should be first in sorted list", "NoSessions",
// leaderboard.getUsers().get(0).getFirstName());
// } // ============ INTEGRATION TESTS ============
...
// "pass", 1, UUID.randomUUID());
// GameSession session = new GameSession(newUser.getUUID(), "IntegrationTeam",
// "IntegrationSession", "Horror", 2, 1);
...
// assertEquals("Should have correct email", "integration@test.com",
// user.getEmail());
// assertFalse("Should have sessions", user.getAllSessions().isEmpty());
// break;
// }
// }
// assertTrue("Should find integration test user after reload",
// foundIntegrationUser);
// }
...
// assertNotNull("Should create leaderboard even with corrupted data",
// corruptedLeaderboard);
...
// User userPositive = new User("Positive", "Score", "pos@test.com", "pass", 1,
// UUID.randomUUID());
// User userNegative = new User("Negative", "Score", "neg@test.com", "pass", 2,
// UUID.randomUUID());

// GameSession positiveSession = new GameSession(userPositive.getUUID(),
// "PosTeam", "PosSession", "Horror", 1, 1);
// positiveSession.setScore(100);
// userPositive.storeGameSession(positiveSession);

// GameSession negativeSession = new GameSession(userNegative.getUUID(),
// "NegTeam", "NegSession", "Fantasy", 1, 1);
...
// assertEquals("Positive score should be ranked higher", "Positive",
// leaderboard.getUsers().get(0).getFirstName());
// assertEquals("Negative score should be ranked lower", "Negative",
// leaderboard.getUsers().get(1).getFirstName());
...
// User userLarge = new User("Large", "Score", "large@test.com", "pass", 1,
// UUID.randomUUID());
// GameSession largeSession = new GameSession(userLarge.getUUID(), "LargeTeam",
// "LargeSession", "Horror", 3, 1);
...
// assertEquals("Should be the correct user", "Large",
// leaderboard.getUsers().get(0).getFirstName());

// // Test that displayLeaderBoard doesn't throw exception (it will reload data
...
// // This is a basic test - real thread safety would require more complex
// testing
// final int initialUserCount = leaderboard.getUsers().size();

// // Simulate multiple operations
// for (int i = 0; i < 10; i++) {
// User user = new User("Concurrent" + i, "User", "concurrent" + i +
// "@test.com", "pass", i, UUID.randomUUID());
// leaderboard.addUser(user);
// }

// assertEquals("Should add all concurrent users", initialUserCount + 10,
// leaderboard.getUsers().size());
...
// assertEquals("Should maintain user count after multiple sorts",
// initialUserCount + 10, leaderboard.getUsers().size());
// }
// }
